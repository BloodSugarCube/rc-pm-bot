...
<p class="muted">Отметьте комнаты с опросом. В «Теги команд» укажите через запятую теги Rocket.Chat Teams (например <code>@developers, @testers</code>) — по ним строится состав для утреннего напоминания и дневного опроса. Если команда в RC пуста, токен считается логином пользователя. Список комнат подтягивается с сервера при открытии страницы.</p>
@if(!empty($syncError))
    ...
    <div class="card">
        <table>
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="5">Нет данных. Проверьте ROCKETCHAT_* в .env и права бота.</td></tr>
                @endforelse
            </tbody>
        </table>
