...
<p class="muted">Отметьте комнаты с опросом — каналы с включённым опросом показываются выше. В «Дневной опрос» включите, нужно ли отправлять дневной опрос и дневное напоминание для канала. В «Теги команд» через запятую укажите теги Rocket.Chat Teams (например <code>@developers, @testers</code>) — по ним строится состав для утреннего и дневного напоминаний и дневного опроса. Если команда в RC пуста, токен считается логином пользователя. В «Не тегать в напоминаниях» перечислите логины, которых не нужно упоминать в этом канале, даже если они в команде (например <code>@aleksandr, @evgenia</code>). Временные исключения по датам — в разделе «Периоды отсутствий». Список комнат подтягивается с сервера при открытии страницы.</p>
@if(!empty($syncError))
